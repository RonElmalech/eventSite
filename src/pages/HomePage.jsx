...
const ConfettiPiece = ({ color = "#ff0000", size = 10, top, left, right, bottom, delay = 0 }) => {
  // Random shape - square, circle or rectangle
  const shape = Math.floor(Math.random() * 3);
  const borderRadius = shape === 0 ? '0%' : shape === 1 ? '50%' : '25%';
  const width = shape === 2 ? size * 1.5 : size;
  ...
  return (
    <MotionBox
      ...
      transition={{
        ...
        repeat: Infinity,
        repeatDelay: 3 + Math.random() * 5 + delay,
        delay: delay,
        ease: "linear"
      }}
    />
...
// Confetti Group Component - creates multiple confetti pieces in an area
const ConfettiGroup = ({ count = 10, area = "top-left", colors = ["#FFC107", "#FF5722", "#E91E63", "#3F51B5", "#4CAF50"] }) => {
  const getPosition = () => {
    switch(area) {
...
const HomePage = () => {
  ...
  useEffect(() => {
    // Check if device is low-end (can be expanded with more checks)
    const isLowEndDevice = () => {
      // Check for slow devices or browsers
    ...
    AOS.init({
      duration: 500, // Even shorter duration
      easing: 'ease',
      once: true,
      offset: 150,
      delay: 0,
      ...
      disable: isLowEndDevice() ? true : 'mobile', // Disable on low-end devices
      throttleDelay: 99,
      debounceDelay: 50,
    ...
    // Refresh AOS after page has fully loaded
  ...
  // Delay initial animations slightly to improve performance
  useEffect(() => {
  ...
  const confettiColors = [
    ...
    "#9C27B0", "#00BCD4", "#FFEB3B", "#FF9800", "#8BC34A"
  ];
  ...
  return (
    <Box sx={{ overflow: 'hidden' }}>
      ...
      >
        ...
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <MotionBox 
            ...
            initial="hidden"
            animate={animationsLoaded ? "visible" : "hidden"}
          >
            <MotionBox 
              component="img"
              src={transparentLogo}
      ...
      >
        ...
        {/* Add confetti in empty areas */}
        <ConfettiGroup area="center-left" count={8} colors={confettiColors} />
        <ConfettiGroup area="center-right" count={8} colors={confettiColors} />
        <ConfettiGroup area="top-center" count={10} colors={confettiColors} />
        ...
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
          ...
          {/* Services Grid - Extra simplified for better performance */}
          <Box 
            data-aos="fade" 
            data-aos-once="true"
            sx={{ 
          ...
          >
            {servicesData.services.map((service, index) => {
              ...
              return (
                <Box
                  key={index}
                  // No AOS on individual cards for smoother scrolling
                  sx={{
                    height: '400px',
                    display: 'flex',
      ...
      {/* Contact CTA Section - This is kept, removing everything between WhyChooseUs and this section */}
      <MotionBox 
        sx={{ 
          py: { xs: 8, md: 10 }, 
      ...
      >
        ...
        {/* Add confetti */}
        <ConfettiGroup area="top-left" count={10} colors={confettiColors} />
        <ConfettiGroup area="bottom-right" count={10} colors={confettiColors} />
        <ConfettiGroup area="bottom-center" count={8} colors={confettiColors} />
